...
/**
 * JdbcPool 数据池（数据源）（获取Connection和，关闭操作）
 ...
 */
public class JdbcPool02 {
    public static Vector<Connection> victors = new Vector();
    private static void loadConfiguration() throws IOException, ClassNotFoundException, SQLException {

            InputStream inputStream = JdbcPool02.class.getClassLoader().getResourceAsStream("db.properties");
...
//                    System.out.println("日志--获取到Connection:" + i);
    ...
    public static Connection getConnection() throws SQLException, IOException, ClassNotFoundException {
        if (victors.size() == 0) {
            JdbcPool02.loadConfiguration();
        }
        if (victors.size() > 0) {
            //取出一个 Connection,并给final？
            final Connection connection = victors.remove(0);
            //关键的动态代理技术，实现修改connection的close方法
            return (Connection) Proxy.newProxyInstance(JdbcPool02.class.getClassLoader(), connection.getClass().getInterfaces(), new InvocationHandler() {
                @Override
